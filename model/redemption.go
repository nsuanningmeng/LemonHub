...
func SearchRedemptions(keyword string, status string, startIdx int, num int, siteScope int) (redemptions []*Redemption, total int64, err error) {
	tx := DB.Begin()
	if tx.Error != nil {
		return nil, 0, tx.Error
	}
...
func Redeem(key string, userId int) (quota int, err error) {
	...
	syncCreditUserQuotaCache(userId, redemption.Quota, "redemption", cacheGeneration)
