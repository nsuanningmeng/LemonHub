...
func (token *Token) Update() (err error) {
	...
	if cacheErr := invalidateTokenCacheForMutation(token.Key); cacheErr != nil {
		return fmt.Errorf("failed to invalidate token cache before update: %w", cacheErr)
	}
...
func (token *Token) SelectUpdate() (err error) {
	if cacheErr := invalidateTokenCacheForMutation(token.Key); cacheErr != nil {
		return fmt.Errorf("failed to invalidate token cache before status update: %w", cacheErr)
	}
...
func (token *Token) Delete() (err error) {
	if cacheErr := invalidateTokenCacheForMutation(token.Key); cacheErr != nil {
		return fmt.Errorf("failed to invalidate token cache before delete: %w", cacheErr)
	}
...
func BatchDeleteTokens(ids []int, userId int) (int, error) {
	...
	if err := invalidateTokensCache(tokens); err != nil {
		tx.Rollback()
		return 0, fmt.Errorf("failed to invalidate token cache before batch delete: %w", err)
	}
