...
// be an absolute upstream URL, or the epay reconciler's gateway query — must also
// call this on the INITIAL URL, which the HTTP client's CheckRedirect hook never
// sees. SSRF protection is ENABLED by default (enable_ssrf_protection); this is a
// no-op only when an operator explicitly disables it.
