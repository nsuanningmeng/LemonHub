...
func setDashboardAuthContext(c *gin.Context, user *model.UserBase, identity service.AuthIdentity, useAccessToken bool) {
	...
	// Operator's own sub-site (0 = main site), read back by EffectiveSiteScope to restrict
	// sub-site admins to their own site and by relay pricing for the sub-site markup. The
	// stateless-token flow authenticates against UserBase, which does not carry site_id,
	// so resolve it from the authoritative account record here. Left unset on lookup
	// failure so EffectiveSiteScope fails closed (SiteScopeDenied) rather than defaulting
	// a scoped operator to main site 0.
	if siteId, err := model.GetUserSiteId(user.Id); err == nil {
		c.Set("operator_site_id", siteId)
	} else {
		common.SysLog(fmt.Sprintf("failed to resolve operator site for user %d: %v", user.Id, err))
	}
	user.WriteContext(c)
}
...
func TokenAuth() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		if len(tokenGroups) > 0 {
			...
			// 主分组 = 最高优先级（列表首项），作为 UsingGroup 默认值（计费/展示）。
