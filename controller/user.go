...
func GetAllUsers(c *gin.Context) {
	...
	users, total, err := model.GetAllUsers(pageInfo, middleware.EffectiveSiteScope(c), sortOptions)
	if err != nil {
		common.ApiError(c, err)
		return
	}
...
func SearchUsers(c *gin.Context) {
	...
	users, total, err := model.SearchUsers(keyword, group, role, status, pageInfo.GetStartIdx(), pageInfo.GetPageSize(), middleware.EffectiveSiteScope(c), sortOptions)
	if err != nil {
		common.ApiError(c, err)
		return
	}
...
func UpdateUser(c *gin.Context) {
	...
	if err := model.DB.Transaction(func(tx *gorm.DB) error {
		if err := updatedUser.EditWithTx(tx, updatePassword, affCommissionProvided, affCashSettledProvided); err != nil {
			return err
		}
