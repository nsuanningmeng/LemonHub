...
func TestSettleTestQuotaSaturationIsAudited(t *testing.T) {
	...
	other := buildTestLogOther(ctx, info, priceData, &dto.Usage{}, result)
...
func TestSelectChannelsForAutomaticTestSkipsAutoTestDisabledChannels(t *testing.T) {
	autoTestDisabledSetting := `{"auto_test_disabled":true}`
	channels := []*model.Channel{
		{Id: 1, Status: common.ChannelStatusEnabled},
		{Id: 2, Status: common.ChannelStatusEnabled, Setting: &autoTestDisabledSetting},
		{Id: 3, Status: common.ChannelStatusAutoDisabled, Setting: &autoTestDisabledSetting},
	}
	...
	recovery := selectChannelsForAutomaticTest(channels, operation_setting.ChannelTestModePassiveRecovery)
...
func TestChannelHealthCheckCountsUnsupportedLocalErrorWithoutAutoDisablePanic(t *testing.T) {
	...
	require.NotPanics(t, func() {
		// A negative threshold guarantees the old implementation entered the
		// timeout branch and dereferenced the local-error result's nil context.
		summary = testChannelForHealthCheck(nil, channel, 1, true, -1)
	})
