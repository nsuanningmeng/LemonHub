...
var auditContentTemplates = map[string]string{
	...
	"user.request_body_record": "${state} request body recording for user ${username} (ID: ${id})",
	"log.request_body_view":    "Viewed the recorded request body for request ${request_id}",
	"log.request_body_clear":   "Cleared all recorded request bodies (${deleted} removed)",
