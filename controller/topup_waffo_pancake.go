...
func getWaffoPancakePayMoney(amount int64, group string) float64 {
	return calculateTopUpPayMoney(amount, setting.WaffoPancakeUnitPrice, group)
}
...
func CreateWaffoPancakePair(c *gin.Context) {
	...
	if err != nil {
		...
		logger.LogError(c.Request.Context(), fmt.Sprintf(
			"Waffo Pancake 创建店铺与产品失败 orphan_store=%t store_id=%q reason=sdk_error",
			orphan, func() string {
				if result == nil {
					return ""
				}
...
func ListWaffoPancakeCatalog(c *gin.Context) {
	...
	// accepted in a URL, where browsers, proxies and access logs retain them.
	...
	if err != nil {
		logger.LogError(c.Request.Context(), "Waffo Pancake 拉取店铺与产品目录失败 reason=sdk_error")
...
func CreateWaffoPancakeSubscriptionProduct(c *gin.Context) {
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf(
			"Waffo Pancake 创建套餐产品失败 store_id=%q reason=sdk_error",
			storeID,
		))
...
func ListWaffoPancakeSubscriptionProductOptions(c *gin.Context) {
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf(
			"Waffo Pancake 拉取订阅产品列表失败 store_id=%q reason=sdk_error", storeID,
		))
...
func RequestWaffoPancakePay(c *gin.Context) {
	...
	if err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo Pancake 创建结账会话失败 user_id=%d trade_no=%s reason=sdk_error", id, tradeNo))
		topUp.Status = common.TopUpStatusFailed
		_ = topUp.Update()
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "拉起支付失败"})
		return
	}
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo Pancake 充值订单创建成功 user_id=%d trade_no=%s session_id=%q amount=%d money=%.2f", id, tradeNo, session.SessionID, req.Amount, payMoney))
...
func WaffoPancakeWebhook(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo Pancake webhook 收到请求 path=%q client_ip=%s body_bytes=%d", c.Request.URL.Path, c.ClientIP(), len(bodyBytes)))

	event, err := service.VerifyConfiguredWaffoPancakeWebhook(string(bodyBytes), signature)
	if err != nil {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Waffo Pancake webhook 验签失败 path=%q client_ip=%s body_bytes=%d reason=invalid_signature", c.Request.URL.Path, c.ClientIP(), len(bodyBytes)))
	...
	if !strings.EqualFold(strings.TrimSpace(event.Mode), expectedEnv) {
		logger.LogError(c.Request.Context(), fmt.Sprintf(
			"Waffo Pancake webhook 环境不匹配 expected=%q actual_mode=%q event_id=%q order_id=%q client_ip=%s",
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo Pancake webhook 验签成功 event_type=%q event_id=%q order_id=%q client_ip=%s", event.NormalizedEventType(), event.ID, event.Data.OrderID, c.ClientIP()))
	...
	if isSubscription {
		...
		if err != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf(
				"Waffo Pancake webhook 订阅订单解析失败 event_id=%q order_id=%q client_ip=%s reason=order_resolution_failed",
				event.ID, event.Data.OrderID, c.ClientIP(),
			))
			c.String(http.StatusOK, "OK")
			return
		}
		...
		if err := model.CompleteSubscriptionOrder(tradeNo, string(bodyBytes), model.PaymentProviderWaffoPancake, ""); err != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo Pancake 订阅完成失败 trade_no=%q event_id=%q order_id=%q client_ip=%s error=%q", tradeNo, event.ID, event.Data.OrderID, c.ClientIP(), err.Error()))
			c.String(http.StatusInternalServerError, "retry")
			return
		}
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo Pancake 订阅完成 trade_no=%q event_id=%q order_id=%q client_ip=%s", tradeNo, event.ID, event.Data.OrderID, c.ClientIP()))
		c.String(http.StatusOK, "OK")
		return
	}
	...
	if err != nil {
		...
		logger.LogError(c.Request.Context(), fmt.Sprintf(
			"Waffo Pancake webhook 订单解析失败 event_id=%q order_id=%q client_ip=%s reason=order_resolution_failed",
			event.ID, event.Data.OrderID, c.ClientIP(),
		))
		c.String(http.StatusOK, "OK")
		return
	...
	if err := model.RechargeWaffoPancake(tradeNo); err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo Pancake 充值处理失败 trade_no=%q event_id=%q order_id=%q client_ip=%s error=%q", tradeNo, event.ID, event.Data.OrderID, c.ClientIP(), err.Error()))
		c.String(http.StatusInternalServerError, "retry")
		return
	}

	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo Pancake 充值成功 trade_no=%q event_id=%q order_id=%q client_ip=%s", tradeNo, event.ID, event.Data.OrderID, c.ClientIP()))
	c.String(http.StatusOK, "OK")
}
