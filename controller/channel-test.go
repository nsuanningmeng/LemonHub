...
func settleTestQuota(info *relaycommon.RelayInfo, priceData hosttypes.PriceData, usage *dto.Usage) (int, *billingexpr.TieredResult) {
	...
	if !priceData.UsePrice {
		...
		baseQuota, clamp := common.QuotaFromFloatChecked(float64(usage.PromptTokens) + float64(completionQuota))
		if info != nil && info.QuotaClamp == nil && clamp != nil {
			info.QuotaClamp = clamp
		}
...
func testChannelForHealthCheck(ctx context.Context, channel *model.Channel, testUserID int, allowDisable bool, disableThreshold int64) channelTestSummary {
	...
	if result.localErr != nil && result.newAPIError == nil {
		// Local setup/unsupported errors are failed tests, not healthy upstream
		// responses. They also have no relay context, so they must not enter the
		// latency auto-disable or recovery paths below.
		summary.Failed++
		return summary
	}
...
func selectChannelsForAutomaticTest(channels []*model.Channel, mode string) []*model.Channel {
	...
	for _, channel := range channels {
		...
		if channel.GetSetting().AutoTestDisabled {
			continue
		}
