...
- Never convert a computed quota or token count to `int` with a bare cast like `int(float64(quota) * ratio)`, `int(math.Round(...))` on unbounded input, or `int(decimal.IntPart())`. All quota rounding/conversion is centralized in `common/quota_math.go`; use those helpers: `common.QuotaFromFloat` (truncating) for float products, `common.QuotaRound` (half-away-from-zero) where rounding is intended, and `common.QuotaFromDecimal` for decimal products. `billingexpr.QuotaRound` delegates to `common.QuotaRound`. Do not reintroduce local conversion helpers or bare casts. Per-request billing and log quota values remain int32-bounded, and every clamp/NaN fallback is logged via `common.SysError` since a single request should never approach those bounds. Wallet balances, accumulated usage, and referral balances use 64-bit database storage on supported amd64/arm64 builds. Recharge credits use `common.WalletQuotaFromDecimalStrict` and `common.MaxWalletQuota` (2^53-1, exact in browser JavaScript and Redis Lua), with atomic total-balance checks; never use the per-request int32 helpers to limit a wallet recharge or total wallet balance. Token `remain_quota`/`used_quota` balances use the host `int`/database 64-bit storage domain, while each individual API billing delta remains int32-bounded. Do not use the int32 quota conversion helpers to derive or validate a token's total balance limit.
