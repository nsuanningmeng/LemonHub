...
describe('time range expression generation', () => {
  ...
  test('does not broaden a mixed rule when its time restriction is invalid', () => {
    const group: RequestRuleGroup = {
      conditions: [
        ...
        timeCondition({ rangeStart: '9', rangeEnd: '24' }),
      ],
      multiplier: '2',
    }

    expect(buildRequestRuleExpr([group])).toBe('')
  })
...
describe('time range round-trip stability', () => {
  test.each([
    ...
    '((hour("UTC") >= 9 || hour("UTC") < 12) && param("service_tier") == "fast" ? 2 : 1)',
    '(hour("UTC") >= 21 || hour("UTC") < 6 && param("service_tier") == "fast" ? 2 : 1)',
  ])(
    'keeps an explicit OR that cannot be represented visually in raw mode: %s',
    (expr) => {
      expect(tryParseRequestRuleExpr(expr)).toBeNull()
    }
  )
  ...
  test.each([
    ...
    '(param("service_tier") == "fast" && hour("UTC") >= 21 && hour("UTC") < 6 ? 2 : 1)',
  ])(
    'keeps contradictory explicit AND bounds instead of converting them to overnight: %s',
    (expr) => {
      const parsed = tryParseRequestRuleExpr(expr)
      expect(parsed).not.toBeNull()
      expect(
        parsed?.[0].conditions.slice(-2).map((condition) => condition.mode)
      ).toEqual([MATCH_GTE, MATCH_LT])
      expect(buildRequestRuleExpr(parsed ?? [])).toBe(expr)
    }
  )

  test.each([
    '(hour("UTC") >= 0 && hour("UTC") < 23 ? 2 : 1)',
    '(hour("UTC") >= 23 || hour("UTC") < 0 ? 2 : 1)',
    '(hour("UTC") >= 9 && hour("UTC") < 9 ? 2 : 1)',
    '((hour("UTC") >= 21 || hour("UTC") < 6) && param("service_tier") == "fast" ? 3 : 1)',
    '(hour("UTC") >= 9 && hour("Asia/Shanghai") < 12 ? 2 : 1)',
  ])(
    'preserves boundary values, timezone and boolean grouping after editing: %s',
    (expr) => {
      const parsed = tryParseRequestRuleExpr(expr)
      expect(parsed).not.toBeNull()
      expect(buildRequestRuleExpr(parsed ?? [])).toBe(expr)
