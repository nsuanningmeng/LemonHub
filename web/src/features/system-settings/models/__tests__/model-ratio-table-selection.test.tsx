...
it('keeps the drag-selected model name highlighted when the row click opens the editor', async () => {
  ...
  expect(
    await screen.findByRole('heading', { name: 'Edit model pricing' })
  ).toBeInTheDocument()
...
it.each([
  ...
  'deleting %s leaves the gpt-4.1-mini editor open: %s',
  async (deletedModel, remainsOpen) => {
    const onChange = vi.fn()
    const user = userEvent.setup()
    await renderEditor(onChange)
    const editedRow = screen.getByRole('row', { name: /gpt-4\.1-mini/ })
    await user.click(within(editedRow).getByRole('button', { name: 'Edit' }))
    expect(
      screen.getByRole('heading', { name: 'Edit model pricing' })
    ).toBeInTheDocument()

    const deletedRow = screen.getByRole('row', {
      name: new RegExp(deletedModel),
    })
    await user.click(
      within(deletedRow).getByRole('button', { name: 'Open menu' })
    )
