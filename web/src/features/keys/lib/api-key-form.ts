...
export function getApiKeyFormSchema(t: TFunction, maxAutoGroups = 5) {
  ...
  return z
    ...
    .superRefine((data, ctx) => {
      // Auto-group customization only applies when the sole selected group
      // is 'auto'; multi-group priority lists bypass the auto pipeline.
...
export function transformFormDataToPayload(
  ...
  return {
    ...
    // 多分组按优先级顺序逗号拼接（首项 = 最高优先级）。
    group: (data.groups ?? []).join(','),
    // auto_groups 仅对「单个 auto 分组」+ 自定义模式有意义。
    auto_groups:
      ...
      data.auto_groups_mode === 'custom'
        ? data.auto_groups
        : [],
    // cross_group_retry 仅对「单个 auto 分组」有意义；多分组失败转移由后端隐式启用。
    cross_group_retry:
      (data.groups ?? []).length === 1 && data.groups?.[0] === 'auto'
