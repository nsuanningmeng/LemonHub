...
describe('API key Auto group form mapping', () => {
  ...
  test('requires between one and eight ordered token groups', () => {
    ...
    const emptyResult = getApiKeyFormSchema(t).safeParse({
      ...defaults,
      groups: [],
    })
    expect(emptyResult.success).toBe(false)
    if (!emptyResult.success) {
      expect(emptyResult.error.issues[0]?.path).toEqual(['groups'])
      expect(emptyResult.error.issues[0]?.message).toBe('Select a group')
    }

    const oversizedResult = getApiKeyFormSchema(t).safeParse({
      ...defaults,
      groups: Array.from({ length: 9 }, (_, index) => `group-${index}`),
    })
    expect(oversizedResult.success).toBe(false)
    if (!oversizedResult.success) {
      expect(oversizedResult.error.issues[0]?.path).toEqual(['groups'])
      expect(oversizedResult.error.issues[0]?.message).toBe(
        'Maximum 8 groups selected'
      )
    }
  })
})
