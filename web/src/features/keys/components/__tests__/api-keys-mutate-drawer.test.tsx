...
function getFormItemByLabel(labelText: string): HTMLElement {
  const label = [...document.querySelectorAll<HTMLLabelElement>('label')].find(
    (candidate) => candidate.textContent?.trim() === labelText
  )
  const formItem = label?.closest<HTMLElement>('[data-slot="form-item"]')
  if (!formItem) {
    throw new Error(`Expected form item for label "${labelText}"`)
  }
  return formItem
}
...
describe('API keys mutate drawer Auto group integration', () => {
  ...
  test('preserves an unsaved custom order and mode after Auto to ordinary to Auto changes', async () => {
    ...
    const groupFormItem = getFormItemByLabel('Groups')
    const removeSelectedGroup = () => {
      const removeButton = groupFormItem.querySelector<HTMLButtonElement>(
        'button[aria-label="Remove"]'
      )
      if (!removeButton) {
        throw new Error('Expected selected group remove button')
      }
      fireEvent.click(removeButton)
    }
    const getAddGroupTrigger = () => {
      const trigger = groupFormItem.querySelector<HTMLButtonElement>(
        'button[role="combobox"]'
      )
      if (!trigger) {
        throw new Error('Expected group add combobox')
      }
      return trigger
    }

    selectComboboxOption(getAddGroupTrigger(), 'Standard access')
    expect(document.querySelector('button[aria-label="Remove vip"]')).toBe(null)
    removeSelectedGroup()
    selectComboboxOption(getAddGroupTrigger(), 'Automatic routing')
    removeSelectedGroup()
