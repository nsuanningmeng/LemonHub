...
export function ConnectAppDialog({
  ...
  const [selectedTool, setSelectedTool] = useState<ClientIntegration | null>(
    null
  )
          ...
          <div className='grid grid-cols-2 gap-3 sm:grid-cols-3'>
            {tools.map((tool) => (
              <button
                ...
                className='bg-card hover:border-primary focus:ring-primary flex flex-col items-center gap-3 rounded-xl border p-4 transition-all hover:shadow-md focus:ring-2 focus:outline-none'
              >
                <div className='bg-primary/5 text-primary flex h-12 w-12 items-center justify-center rounded-lg'>
                  {tool.family === 'cli' ? (
                    <Terminal />
                  ) : tool.family === 'desktop' ? (
                    <Laptop />
  ...
  const renderToolPanel = () => {
    ...
    const artifacts = selectedTool.artifacts.filter(
      (a) => a.platform === 'any' || a.platform === os
    )

    const hasModelPlaceholder = selectedTool.artifacts.some((a) =>
      a.template.includes('{model}')
    )
      ...
      <div className='space-y-6'>
        ...
        <div className='flex flex-col gap-4 rounded-lg bg-amber-50 p-4 text-amber-800 dark:bg-amber-950/30 dark:text-amber-200'>
          <div className='flex items-start gap-3'>
            ...
            <p className='text-sm'>
              {t(
                'This panel reveals your API key. Make sure you are in a private environment.'
              )}
            </p>
          </div>
          <Button
        ...
        {selectedTool.family === 'cli' && (
          <Tabs
            value={os}
            onValueChange={(v) => setOs(v as 'windows' | 'darwin' | 'linux')}
          >
              ...
              {artifact.kind === 'deeplink' && (
                <div className='space-y-2'>
                  <Button
                    ...
                    onClick={() =>
                      window.open(
                        resolveTemplate(artifact.template, true),
                        '_blank'
                      )
                    }
                  >
              ...
              {artifact.kind === 'snippet' && (
                <div className='space-y-2'>
                  ...
                  <div className='group relative'>
                    <pre className='overflow-x-auto rounded-lg bg-zinc-950 p-4 font-mono text-sm text-zinc-50'>
                      {resolveTemplate(artifact.template)}
                    </pre>
                    <div className='absolute top-2 right-2 opacity-0 transition-opacity group-hover:opacity-100'>
                      <CopyButton
                        value={resolveTemplate(artifact.template, true)}
                      />
                    </div>
                  </div>
                </div>
