...
func TestUpstreamRequestErrorForLogDropsCredentialBearingURL(t *testing.T) {
	err := &url.Error{
		Op:  http.MethodPost,
		URL: "https://upstream.example/v1/chat?custom_name=literal-secret",
		Err: errors.New("connection refused"),
	}

	assert.Equal(t, "connection refused", upstreamRequestErrorForLog(err))
}
