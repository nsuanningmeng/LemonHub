...
// These tests exercise route/URL construction with example hostnames that must not
// hit the DNS-resolving SSRF guard in routeURL (fake-ip DNS setups resolve any name
// to 198.18.0.0/15, which the default-on policy rejects as private). The guard itself
// is covered by the fetch-setting validator tests.
