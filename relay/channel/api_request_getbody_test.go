...
func runGoAwayAfterFirstRequestServer(ln net.Listener) <-chan h2ServerResult {
	...
	go func() {
		...
		_, body, err := readH2TestRequest(firstFramer)
		if err != nil {
			res.err = err
			return
		}
		...
		// Keep the first connection alive after GOAWAY. Closing it immediately
		// races the client reading the frame on Windows, where the socket reset
		// can win and turn a retryable GOAWAY into WSAECONNABORTED.
		...
		streamID, body, err := readH2TestRequest(secondFramer)
		if err != nil {
			res.err = err
			return
		}
		...
		// Deliver the assertion data before waiting for the client to close its
		// idle connection. This lets the test finish reading the response and call
		// CloseIdleConnections; only then do the server-side defers close sockets,
