...
// PublicResponseModelName returns the model name that is safe to expose to the
// client. Channel model mapping is an internal routing detail, so a mapped
// response must keep the model name from the user's request. The upstream
// response model remains available to billing and token-counting code through
// UpstreamModelName.
func (info *RelayInfo) PublicResponseModelName(upstreamResponseModel string) string {
	if info == nil {
		return upstreamResponseModel
	}
...
// RewriteModelForPublicResponse replaces model fields that already exist in a
// JSON response while preserving all other provider-specific fields verbatim.
// It is intentionally a no-op unless channel model mapping was applied.
func (info *RelayInfo) RewriteModelForPublicResponse(data []byte, paths ...string) ([]byte, error) {
	if info == nil || info.ChannelMeta == nil || !info.IsModelMapped || info.OriginModelName == "" || len(data) == 0 {
		return data, nil
	}
